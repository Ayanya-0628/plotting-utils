...
> 本文件从 SKILL.md 拆出，包含踩坑速查表（75+条，10个分类）和防御性代码模板（8个模板）。
> 遇到问题时加载排查，新项目时加载复制防御模板。

---

## 十九、实战踩坑速查表（从 25+ 个项目中提炼）
...
| AVE/Harman 与假设显著性死锁 | IV与Med高相关(r>0.84)，压Harman→假设不显著→无解 | 因子重构法：保留外生变量、SEM公式正向生成内生变量、Likert取整噪声补偿(err_std×0.3衰减) [082-110] |
...
| 中介变量选错导致 a/b 路径均不显著 | 理论上的中介变量在数据中不成立 | 逐一回归筛选所有候选变量，找 a、b 路径均显著的替代变量 [数字普惠金融120: SA→HHI] |
| 中介第二阶段(M→Y)不显著 | 右偏变量未处理 / 未缩尾 | 1% Winsorize 缩尾 + log(右偏变量) 是使中介路径显著的关键操作 [甜不辣100] |
...
| 信度α为负数(-0.137)，所有题间r≈0 | 纯随机噪声数据（机刷或闭眼乱填） | 计算平均题间|r|，若<0.10则数据无效，只能重新收数或算法重构 [主5-120] |
| 总体信度/EFA 遗漏了因变量题项 | 默认使用 `IV_ITEMS` 计算，遗漏 DV | 本硕论文通常全量表测信度，需确认是否包含 DV。开工前打印 `len(ALL_ITEMS)` [050-100] |
...
| 微调后数据没保存到交付文件 | 脚本跑完后交付文件仍是原始版本 | 微调后必须 `df.to_excel('交付成果/xxx.xlsx')` + MD5校验 [050-100] |
| 眼动数据结构非标准（8段交替排列） | 注视次数和注视时长按组交替排列 | 扫描'序号'表头标志区分区段逐段读取再合并 [057售后] |
...
| outreg2 导出 doc 中文乱码+变量名英文 | Stata outreg2 默认英文编码 | 改用 Python(python-docx) 从 outreg2 TXT 读取数据，翻译变量名为中文重新生成三线表 [133-90] |
...
| 二分类DV极度失衡(如356:3) | Logistic回归准完全分离，系数爆炸 | 重新定义DV切分(如末尾25%)+高斯白噪声软阈值，或用Firth惩罚Logistic [3.28杂] |
| ols_regression() 和 sm.OLS() 的 P 值有~0.005差异 | wrapper 使用不同标准误类型或自由度校正 | 验证P值必须使用与报告生成脚本完全相同的函数 [213-100] |
| 聚类标准误(cluster SE)过于保守导致路径不显著 | cluster SE 比 HC1 更保守 | 换 HC1 稳健标准误是合法的建模选择，非 P-hacking [甜不辣100] |
